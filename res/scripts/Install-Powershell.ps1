...
<#
.DESCRIPTION
    This cmdlet install WMF 5.1 on Windows OS lesser than 10/2016.

    There is no need to execute this cmdlet on windows OS 10/2016 or latter.
#>
Param(
...
{
    param(
        [Int] $versionThreshold = 5
    )

    if ($PSVersionTable["PSVersion"].Major -ge $versionThreshold){
        Write-Host "[+] Version of PS already equal/bigger than $versionThreshold"
        return
    }
    ...
    # Also, wusa.exe /extract not supported anymore on windows 10 and latter
