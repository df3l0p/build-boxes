...
This cmdlet needs to be executed after the windows machine has been bootstraped.

This cmdlet make sure it repairs the VM so that next steps can be executed.

At the moment of writting, the script
- Disable Windows updates
- Disable Windows Defender
- Deploy fixes for different hypervisors (qemu, virtualbox, ...)
...
function Disable-WindowsUpdates{
    Param(
        [String] $auPath = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate\AU',
        [String] $deliveryOptimizationPath = 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\DeliveryOptimization\Config'
    )
...
Function Disable-WindowsDefender
{
    ...
    $WindowsDefenderPath = 'HKLM:/SOFTWARE/Policies/Microsoft/Windows Defender'
    if (Test-Path $WindowsDefenderPath){
        Set-ItemProperty -Path $WindowsDefenderPath -Name DisableAntiSpyware -Value 1
    }
}

Function Repair-VboxSrvResolutionDelay
{
    Param(
    )
...
'@
    ...
    Write-Output $content | Out-File -Encoding ASCII -Append 'c:\windows\system32\drivers\etc\lmhosts'
}

Function Repair-LibVirtCpuDriver
{
    param(
...
Repair-VboxSrvResolutionDelay
Repair-LibVirtCpuDriver